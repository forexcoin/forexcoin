...
uint256 hashGenesisBlock("0x00000ba78652446372afab79a4b88f41c1be81b3e60f6c4533bacb5a12300dde");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Forexcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Forexcoin Signed Message:\n";
...
{
    // Forexcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1)
    {
       nSubsidy = 5600000000 * COIN;
        }

    // Subsidy is cut in half every 100000 blocks.
    nSubsidy >>= (nHeight / 200000); // Forexcoin subsidy formula.    
	return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 12 * 60; // Forexcoin: every 12 blocks
static const int64 nTargetSpacing = 1 * 60; // Forexcoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Forexcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	if (fTestNet) {
		...
		if (pindexLast->nHeight+1 >= 36) { DiffMode = 2; }
	}
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    if ((pindexNew->nHeight % 120) == 0 || (!fIsInitialDownload && (pindexNew->nHeight % 1440) == 0))
    {
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Forexcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // forexcoin: check that the block satisfies synchronized checkpoint
	...
	// forexcoin: check pending sync-checkpoint
...
{
    // Forexcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
	...
	// forexcoin: ask for pending sync-checkpoint if any
	...
	// forexcoin: if responsible for sync-checkpoint send it
...
bool static LoadBlockIndexDB()
{
	...
	// forexcoin: load hashSyncCheckpoint
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000f8a3cf2da51529654de471a4929cd22e4edc72e13d2cf61dcdba38c023a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "29.05.2014 Forexcoin X11 code ready";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0448bdea76a05c76cd4e5cb248024793599e338e9b3096332729d78f3abc0b887492a9d31e39001d74c539b2518014e049bbde6abb2978b3f59e42909a74c5130d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1401328800;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2091778246;

        if (fTestNet)
        {
            block.nTime    = 1401328800;
            block.nNonce   = 388125128;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x41d076b83932a16c49f74b2564321406844291535456b2ba7d3b5aa032ecb22d"));
	    block.print();
        ...
        try {
			...
			// forexcoin: initialize synchronized checkpoint
    ...
    // forexcoin: if checkpoint master key changed must reset sync-checkpoint
...
string GetWarnings(string strFor)
{
	...
	// forexcoin: if detected invalid checkpoint enter safe mode
...
unsigned char pchMessageStart[4] = { 0xf6, 0xe6, 0xd6, 0xc6 }; // Forexcoin.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // forexcoin: relay sync-checkpoint
        {
    ...
    else if (strCommand == "checkpoint") // forexcoin synchronized checkpoint
    {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ForexcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ForexcoinMiner : ProcessBlock, block not accepted");
    }
