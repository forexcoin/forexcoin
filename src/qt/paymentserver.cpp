...
const int FOREXCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString FOREXCOIN_IPC_PREFIX("forexcoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(FOREXCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(FOREXCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(FOREXCOIN_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click forexcoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start forexcoin: click-to-pay handler");
    else
...
{
    // clicking on forexcoin: URLs creates FileOpen events on the Mac:
