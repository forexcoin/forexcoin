...
{
    QString ret = QString("forexcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non FRX unit input in lnReqAmount, we generate the URI with FRX as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(ForexcoinUnits::format(ForexcoinUnits::FRX, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
